...
for f in files:
    ...
    # html.unescape matters on BOTH sides. Astro writes &#39; (and, depending on
    # the Astro version, &#x27;) into visible text while JSON.stringify keeps the
    # raw character in the JSON-LD, so without unescaping any FAQ answer with an
    # apostrophe in its first 60 characters is a FALSE failure. Re-lost once
    # (agent collision, 09-18) and re-applied 09-19 after a sandbox build threw
    # 8 of these against pages that are demonstrably clean on the live site.
    ...
    for b in re.findall(r'<script type="application/ld\+json">(.*?)</script>', h, re.S):
        ...
        if o.get('@type') == 'FAQPage':
            for q in o.get('mainEntity', []):
                qn = html.unescape(re.sub(r'\s+', ' ', q['name']).strip())
                an = html.unescape(re.sub(r'\s+', ' ', q['acceptedAnswer']['text']).strip())
                if qn not in vis:
